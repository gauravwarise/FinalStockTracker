...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self,stockpicker):
        task = PeriodicTask.objects.filter(name="every-to-seconds")
        if len(task)>0:
            task = task.first()
    ...
    async def connect(self):
        ...
        #Parse queryStrings
        # query_params = parse_qs(self.scope['query_strings'].decode())
        # print(query_params)

        # stockpicker = query_params['stockpicker']

        # Corrected line: use 'query_string' instead of 'query_strings'
